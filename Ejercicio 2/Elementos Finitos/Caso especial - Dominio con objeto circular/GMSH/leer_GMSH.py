...
def guardar_tablas(nombre_archivo, xnod, LaG):
    # Guardar datos de nodos
    with open(nombre_archivo, 'w') as archivo:
        archivo.write('Nodos:\n')
        archivo.write('x\ty\n')
        for i, (x, y) in enumerate(xnod):
            archivo.write(f'{x:.6f}\t{y:.6f}\n')

        # Guardar datos de triangulación
        archivo.write('\nTriangulación:\n')
        archivo.write('Elem\tN1\tN2\tN3\n')
        for i, nodos in enumerate(LaG[:, 1:]):
            archivo.write(f'{i + 1}\t{nodos[0] + 1}\t{nodos[1] + 1}\t{nodos[2] + 1}\n')